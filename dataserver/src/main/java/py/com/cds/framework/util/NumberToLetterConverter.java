...
public abstract class NumberToLetterConverter {
	...
	public static String convertNumberToLetter(String number) throws NumberFormatException {
		return convertNumberToLetter(Long.parseLong(number));
	}
	 ...
	 */
	public static String convertNumberToLetter(long longNumber) throws NumberFormatException {

		StringBuilder converted = new StringBuilder();

		// Validamos que sea un numero legal
		if (longNumber > 999999999)
			throw new NumberFormatException("El numero es mayor de 999'999.999, " + "no es posible convertirlo");

		if (longNumber < 0)
			throw new NumberFormatException("El numero debe ser positivo");

		String splitNumber[] = String.valueOf(longNumber).replace('.', '#').split("#");
		...
		if (millon >= 1) {
			...
			if (miles > 1)
				converted.append("").append(convertNumber(String.valueOf(miles)) + "MIL ");
		}
